...
class ViNT_Dataset(Dataset):
    def __init__(
        ...
        '''
        with open(traj_names_file, "r", encoding="utf-8-sig") as f:
            file_lines = f.read()
            self.traj_names = file_lines.split("\n")
        '''
                
        # 以二进制方式读取，兼容各种 BOM
        raw = open(traj_names_file, "rb").read()

        # 依次尝试几种常见编码（UTF-8、带BOM的UTF-8、UTF-16 LE/BE）
        ...
        # 统一换行、去空白、过滤注释/空行
        text = text.replace("\r\n", "\n").replace("\r", "\n")
        self.traj_names = [ln.strip() for ln in text.split("\n") if ln.strip() and not ln.lstrip().startswith("#")]
    ...
    def _compute_actions(self, traj_data, curr_time, goal_time):
        ...
        # The yaw value must be a scalar float. If the data is malformed (e.g., [yaw] instead of yaw),
        # extract the scalar value using .item() to make the code more robust.
        current_yaw = yaw[0]
        if hasattr(current_yaw, 'item'):
            current_yaw = current_yaw.item()

        waypoints = to_local_coords(positions, positions[0], current_yaw)
        goal_pos = to_local_coords(goal_pos, positions[0], current_yaw)
        ...
        # Final sanitization to prevent downstream type errors with torch.as_tensor.
        # This handles cases where the numpy array has dtype=object due to malformed pkl data.
        if actions.dtype == np.object_:
            actions = np.array(actions.tolist()).astype(np.float32)

        if goal_pos.dtype == np.object_:
            goal_pos = np.array(goal_pos.tolist()).astype(np.float32)
